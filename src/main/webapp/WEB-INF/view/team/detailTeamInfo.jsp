																				...
																				function(
																					...
																					if (data == "true") {
																						if (confirm("정말로 팀을 해체 하시겠습니까?")) {
																							...
																							location.href = "/ShootBoy/doDeleteTeam?teamId="
																									+ "${teamInfo.teamId}";
																						}
																					} else {
																						alert("비밀번호가 틀렸습니다.")
																				...
																				function(
																					...
																					if (data == "true") {
																						if (confirm("정말로 팀을 탈퇴 하시겠습니까?")) {
																							alert("정상적으로 처리 되었습니다.");
																							location.href = "/ShootBoy/doDrop?teamId="
																									+ "${teamInfo.teamId}";
																						}
																					} else {
																						alert("비밀번호가 틀렸습니다.")
...
<div id="myPageLeftMenu">
	...
	<div id="myPageLeftMenuTwo">
		<div class="mplmOne">
			<a href="/ShootBoy/userInfo">내 정보</a>
		</div>
		<div class="mplmTwo">
			<img src="/ShootBoy/img/화살표.jpg" class="arrow"><a
				href="/ShootBoy/detailTeamInfo">팀 정보</a>
		</div>
		<div class="mplmThree">
			<a href="/ShootBoy/teamMatchInfo">팀 매치 정보</a>
		</div>
		<div class="mplmFour">
			<a href="/ShootBoy/message">메시지함</a>
		</div>
	</div>
</div>

<div class="myInfoText" style="display: inline-block; width: 800px;">
	<h1>팀 정보</h1>
	<hr class="myPageline">
		...
		<c:if test="${!empty userInfo.teamId}">
			...
			<div id="teamDetailContainer"
				...
				<div class="teamName" style="font-size: 20px; padding-bottom: 10px">팀명
					: ${userInfo.teamVO.teamName}</div>
				<div class="teamCount" style="font-size: 20px; padding-bottom: 10px">팀원수
					: ${userInfo.teamVO.teamCount}</div>
				<div class="teamPoints"
					style="font-size: 20px; padding-bottom: 10px">팀 포인트 :
					${userInfo.teamVO.teamPoint}</div>
				<div class="teamCreatedDate"
					style="font-size: 20px; padding-bottom: 10px">팀 생성일 :
					${userInfo.teamVO.createDate}</div>
				<div class="teamLocation"
					style="font-size: 20px; padding-bottom: 10px">팀 지역 :
					${teamInfo.locationVO.parentLocationName} -
			...
			<div id="teamModify" style="float: right; margin-right: 100px;">
				...
				<c:choose>
					<c:when test="${userInfo.levelId eq '3'}">
						<div>
							<input type="button" id="deleteTeam" name="deleteTeam"
								value="팀 해체" />
						</div>
					</c:when>
					<c:otherwise>
						<div>
							<input type="button" id="dropTeam" name="dropTeam" value="팀 탈퇴" />
						</div>
					</c:otherwise>
				</c:choose>
				...
				<div>
					<input type="password" id="password" name="password"
						placeholder="비밀번호를 입력해주세요"> <input type="button"
						id="passwordConfirm" name="passwordConfirm" value="비밀번호 확인"
